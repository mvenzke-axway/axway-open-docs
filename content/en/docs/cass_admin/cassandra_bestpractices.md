...
1. Execute the following using `cqlsh` on one of the Cassandra nodes:
   ...
   {{< alert title="Caution" color="danger" >}}Cassandra will not keep the TTL modification on system tables when restarting, so you must execute these commands at each cassandra restart.{{< / alert >}}
